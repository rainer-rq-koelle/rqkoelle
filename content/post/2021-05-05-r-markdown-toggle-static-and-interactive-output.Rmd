...
For `{plotly}` there is another workaround using `{webshot}` or `{webshot2}` and `phantom.js` as described in this [older post](2021-02-01-plotly-to-static-image-for-pdf-or-word/). I played around with it and it works. My struggle were to get phantom.js running on my work laptop (no admin rights & IT security settings requiring libraries in certain folders).     
